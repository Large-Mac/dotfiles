...
void getcmd(const Block *block, char *output)
{
  ...
  if (block->signal) //Added for fix
		tempstatus[0] = block->signal;    // Add this line to handle signal byte
	strcpy(tempstatus + (block->signal ? 1 : 0), block->icon);  // Modify this line to account for signal byte
